# Portfólio de casos reais de Engenharia e Análise de dados
Cases reais de projetos que tive a oportunidade atuar enquanto engenheiro de dados e analista de dados.
## Cases




| Caso | Descrição |
|------|-----------|
| 🚀 [CASE 01 - E-Financeira](01-case-e-financeira/README.md) | Pipeline de consolidação de dados para geração da e-Financeira |
| 📊 [CASE 02 - Unificação de Produtos de Investimentos](02-case-unificacao-produtos-letras/README.md) | Consolidação de produtos de investimentos (LCA e LCI) provenientes de dois Core Banking, transformando dados da camada Silver em uma tabela Gold para consumo analítico via Denodo, Power BI e consultas de negócio. |
| 📊 [CASE 03 - Dashboard Aplicação LCA](03-case-dashboard-lca/README.md) | Desenvolvimento de solução analítica em Power BI para monitoramento das aplicações em LCA, saldo em carteira e disponibilidade de lastro.|
| 📊 [CASE 04 - Dashboard para Supermercados](04-case-bi-para-supermercado/README.md) | Desenvolvimento de solução analítica em Power BI para monitoramento das aplicações em LCA, saldo em carteira e disponibilidade de lastro.|


---
> **Observação:** Estes projetos utiliza dados fictícios e estruturas anonimizadas, desenvolvidos exclusivamente para fins de demonstração técnica. Nenhuma informação confidencial, proprietária ou de clientes reais é apresentada neste repositório.